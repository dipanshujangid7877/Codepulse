...
function Pagination({
  ...
  const [defaultOffsetStartIndex, setDefaultOffsetStartIndex] = React.useState(defaultOffset);
  const [offsetStartIndex, setOffsetStartIndex] = React.useState(defaultOffsetStartIndex);
...
function Range({
  ...
  }, start, /*#__PURE__*/React__default.default.createElement(VisuallyHidden.VisuallyHidden, null, "\xA0through\xA0"), /*#__PURE__*/React__default.default.createElement("span", {
    "aria-hidden": "true"
  }, "\u2012"), end, " of ", totalCount));
}
...
function usePagination(config) {
  ...
  const [defaultIndex, setDefaultIndex] = React.useState(() => {
    if (defaultPageIndex !== undefined) {
      ...
      '<Pagination> expected `defaultPageIndex` to be less than the ' + 'total number of pages. Instead, received a `defaultPageIndex` ' + 'of %s with %s total pages.', defaultPageIndex, pageCount) : undefined;
    }
    return 0;
  });
